...
class CalibrateStereoCamerasFromChessboard:
    ...
    def execute(self, image_set):
        images_left = sorted(
            glob.glob('../code/bin/sets/' + image_set + '/calibration_images/left*' + self.IMAGE_FORMAT))
        images_right = sorted(
            glob.glob('../code/bin/sets/' + image_set + '/calibration_images/right*' + self.IMAGE_FORMAT))
        ...
        return self.checkBackprojectError(cameraMatrix1, cameraMatrix2, distCoeffs1, distCoeffs2, ret1, ret2, rvecs1,
                                          rvecs2,
                                          stereocalib_retval, tvecs1, tvecs2, object_points, image_points_a,
                                          image_points_b)
    ...
    def checkBackprojectError(self, cameraMatrix1, cameraMatrix2, distCoeffs1, distCoeffs2, ret1, ret2, rvecs1, rvecs2,
        ...
        return {
            "total error left": total_error_a / len(object_points),
            "total error right": total_error_b / len(object_points),
            'ret1': ret1,
            "ret2": ret2,
            "retval": stereocalib_retval
        }
    ...
    def stereo_calibration(self, object_points, image_points_a, image_points_b):
        ...
        ret1, camera_matrix_a, distortion_coefficients_a, rvecs1, tvecs1 = cv2.calibrateCamera(object_points,
                                                                                               image_points_a,
                                                                                               self.IMAGES_SHAPE[::-1],
                                                                                               None, None)
        ret2, camera_matrix_b, distortion_coefficients_b, rvecs2, tvecs2 = cv2.calibrateCamera(object_points,
                                                                                               image_points_b,
                                                                                               self.IMAGES_SHAPE[::-1],
                                                                                               None, None)
    ...
    def extract_chessboard_points(self, images_a_paths, images_b_paths):
        ...
        for image_a_path, image_b_path in zip(images_a_paths, images_b_paths):
            ...
            if corners_in_image_a_found and corners_in_image_b_found:
                image_a_corners = cv2.cornerSubPix(image_a_gray, image_a_corners, (11, 11), (-1, -1),
                                                   self.STOP_CRITERIA)
                image_b_corners = cv2.cornerSubPix(image_b_gray, image_b_corners, (11, 11), (-1, -1),
                                                   self.STOP_CRITERIA)
